...
public class DeptRestController {
    ...
    @GetMapping("/{no}") // pathvariable쓸려면 중괄호 써서 사용해야함
...
//        return readService.getDept(deptno);
    ...
    @PostMapping // == /api/v1/depts 이 경로임 위에랑은 차이점이 여기서는 Post형식임
    public ResponseEntity<String> regDept(@RequestBody DeptRegistRequest registRequest) {
        // JSON으로 넘어오는 데이터를 자바에 객체로 받아야함.
        ...
        // insert Ok or Insert FAIl (결과값 예상)

    }

    @PutMapping("/{no}") // "/{no}"->pathvariable임
