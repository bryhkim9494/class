...
public class FileUploadController {
    ...
    public String getReportForm() {
        ...
        log.info("GET  /report"); //@RequestMapping("/report")
        return "report/reportForm";
    }


    // post : 파일 처리 -> 파일 이름확인, 사이즈 , 파일 저장
    // 파일 저장: 저장 폴더 생성해서 해당 경로(웹경로) -> 시스템 경로에 파일을 저장
    // 이미지를 업로드 -> 웹페이지에서 확인

    //    @RequestMapping(value = "/submit1", method = RequestMethod.POST)
    @PostMapping("/submit1") // == 경로 : /report/submit1
    ...
    @PostMapping("/submit2")
    public String submit2(MultipartHttpServletRequest request, Model model) throws IOException {
        ...
        // 실제 경로
        String dirRealPath = request.getSession().getServletContext().getRealPath(uploadURI); // == getRealPath("/uploadfile/report");
    ...
    @PostMapping("/submit3")
    public String submit3(Report report, HttpServletRequest request) throws IOException {
        ...
        // 파일 저장
        report.getReport().transferTo(newFile);// 커맨드 객체(report만 정확히 알면됨)
